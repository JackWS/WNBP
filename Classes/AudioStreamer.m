...
- (void)scheduleStreamResetTimer;
- (void)invalidateStreamResetTimer;
@end
...
//																			
		[self presentAlertWithTitle:NSLocalizedStringFromTable(	@"Radio Error", 	// key
																@"Errors", 			// table
																@"File Error")		// comment
							message:NSLocalizedStringFromTable(	@"Unable to play network radio", 				// key
																@"Errors", 										// table
																@"Unable to configure network read stream.")];	// comment
	}
}
...
- (BOOL)openReadStream
{
	@synchronized(self)
	{
		...
		{
			CFRelease(stream);
			[self presentAlertWithTitle:NSLocalizedStringFromTable(@"Radio Error", @"Errors", nil)
...
)
{
	...
	switch ( routeChangeReason )
	{
		...
		case kAudioSessionRouteChangeReason_OldDeviceUnavailable:
			JSLog(@"headset unplugged");
			[streamer pause];
			break;
			
		case kAudioSessionRouteChangeReason_NewDeviceAvailable:
			JSLog(@"headset plugged in ");
			break;
			
		default:
			JSLog(@"audio route change reason: %d", routeChangeReason);
			break;
	}
}
...
- (void)stop
{
	[self invalidateStreamResetTimer];
	@synchronized(self)
	{
...
{
	NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
	UIBackgroundTaskIdentifier newTaskId = &UIBackgroundTaskInvalid ? UIBackgroundTaskInvalid : -1;
	...
	@synchronized(self)
	{
		if (inID == kAudioQueueProperty_IsRunning)
		{
			...
			else if (state == AS_WAITING_FOR_QUEUE_TO_START)
			{
				...
				{				
					if ( (&UIBackgroundTaskInvalid) && (bgTaskId != UIBackgroundTaskInvalid)) {
...
- (void)scheduleStreamResetTimer
{
	if ( streamResetTimer == nil )
	{
		CexiMeAppDelegate* appDelegate = (CexiMeAppDelegate*)[[UIApplication sharedApplication] delegate];
	
		streamResetTimer = [NSTimer scheduledTimerWithTimeInterval:appDelegate.settingsManager.streamRefresh
																	 target:self 
																   selector:@selector(restartStream:) 
																   userInfo:nil 
																	repeats:NO];
	}	
}
...
- (void)invalidateStreamResetTimer
{
	if ( streamResetTimer )
	{
		[streamResetTimer invalidate];
		streamResetTimer = nil;
	}
}

// Restart stream periodically
- (void)restartStream:(NSTimer *)timer 
{
	JSLog(@"Restarting stream");
	[self stop];
	[self start];
}
